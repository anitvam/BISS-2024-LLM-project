...
neural network, input tokens are transformed into contextual embeddings. Notably, we are only interested in
...
The input passes through multiple decoder blocks, each composed by a masked self-attention layer and a
feed-forward neural network. The masking makes it so each token only attends to previous tokens in the
sequence, maintaining a unidirectional context. The final hidden state, corresponding to the last token in the
sequence, is fed to the linear layer on top of the stack of decoders. Its output dimension is equal to the number
of labels of the given task.
%
...
\begin{tabular}{ ll  }
  ...
  \hline
   ...
   \hline
   \multicolumn{2}{|c|}{Model proposed with no fine-tuning} \\
   \hline
  ...
  \hline
   \multicolumn{2}{|c|}{Model proposed after fine-tuning}\\
   \hline
